...
public class NewMessageSentConsumer {
    ...
    public void consumeMessage(ViewProfileMessageDto message) {
        notificationsRepository.save(new Notification(message.getUserId(), message.getViewerId(), message.getViewerName()+" has viewed your profile",  new Timestamp(System.currentTimeMillis()), false));
    }
}
