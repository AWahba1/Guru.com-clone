package com.feedbackApp.Controller;

import com.feedbackApp.Models.Feedback_By_Client;
import com.feedbackApp.Models.Feedback_By_Client_Pkey;
import org.springframework.data.cassandra.repository.CassandraRepository;
import org.springframework.data.cassandra.repository.Query;

import java.sql.Timestamp;
import java.util.List;
import java.util.UUID;

public interface Feedback_By_ClientRepository extends CassandraRepository<Feedback_By_Client, Feedback_By_Client_Pkey> {
    @Query("SELECT * FROM Feedback_By_Client WHERE client_id = ?0 ORDER BY created_at DESC")
    List<Feedback_By_Client> findByCompositeKey(UUID client_id);

    @Query("UPDATE Feedback_By_Client SET satisfied = ?3 WHERE client_id = ?0 AND created_at = ?1 AND feedback_id = ?2")
    void updatefeedbackClient(UUID client_id ,Timestamp created_at, UUID feedback_id, Boolean satisfied);

    @Query("DELETE FROM Feedback_By_Client WHERE client_id = ?0 AND created_at = ?1 AND feedback_id = ?2")
    void deletefeedbackClient(UUID client_id, Timestamp created_at, UUID feedback_id);








}
